...
> 历史设计文档（PRD/HLD/TDD/LLD-01/LLD-04）已完成使命，归档于 `docs/archive/`，不再维护。本文档为架构权威参考；接口细节以 `src/` 源码为准。
...
### 3.3 隔离策略 `IsolationPolicy`（Phase 16 收敛）
...
旧值 `None` / `LoopbackOnly` / `Outbound` 与旧字段 `fs_mode` / `capabilities` / `path_rules` / `filesystem` 在解析层显式拒绝（AppContainer 移除后无法在纯用户态下生效，杜绝"配了没生效"的静默失败）。
...
| 全盘可读可执行 | 默认无 `NO_READ_UP`，保留用户 SID/组，用户可读文件均可读可执行 |
...
| `on_job_process_started` | Job 内子/孙进程创建（Phase 9） | `process_id` / `pid` / `process_name` / `process_path` / `parent_pid?` / `timestamp_ms` |
| `on_job_process_exited` | Job 内子/孙进程退出（Phase 9） | `process_id` / `pid` / `exit_kind` / `exit_code?` / `timestamp_ms` |
| `on_behavior_event` | ETW 行为事件批量上报（Phase 6） | `events`（list[dict]） |
...
8. Execute（usecase 内部）：
   ...
   e. ProcessLauncher::Launch（CreateProcessAsUserW(隔离 token) + stdio 管道/ConPTY）
...
9. 成功后插入 processes_ 映射
```

异常路径要点：

- **Execute 失败**：先 `Job::Shutdown()` 停通知线程再让 entry 析构，避免 IOCP 回调访问悬垂 sink（BUG-1 修复，见 7.9）
...
- **wall_clock 超时**：Python 端 `WallClockTimer` 上下文管理器超时回调 `proc.terminate(1)`。退出原因与手动终止区分（H-reason 修复）
...
Phase 9 起，`NewProcess` / `ProcessExitNormal` / `ProcessExitAbnormal` / `ProcessExit` 四类通知除日志外，还由 `NativeSandboxedProcess::OnNotification` 透传为 `on_job_process_started` / `on_job_process_exited` 回调（主进程 pid 跳过，避免与 `wait()` 返回重复；`NewProcess` 时经 Toolhelp 尽力填充 `parent_pid`）。

两个修复记录（勿回退）：

- **H-1/H-2 修复**：曾仅设 reason + 发回调而不杀进程，导致 64MB 配额下 80MB 进程照常跑完（exit_code=0）、cpu_ms=500 实耗 1124ms。修复为通知后立即 TerminateAll
- **2026-08-09 修复**：`ActiveProcessLimit` 曾与 CPU/内存同路径 TerminateAll，但它是"创建时拒绝"语义，Job 内既有进程并未违规，杀它们是过激行为——曾导致测试脚本主进程未写完 stdout 即被杀

### 5.3 Low IL token 隔离（Phase 16，替换原 AppContainer 链）
...
- **plain 单路径（实测定案）**：不 CreateRestrictedToken —— restricted token 由非管理员宿主启动必 err=1314（三路实验定案）；隔离 token 特权集 = 宿主镜像（非管理员仅 5 个无害特权），隔离核心全部来自 IL 完整性强制
- **零特权可用**：token 派生（SeImpersonatePrivilege 默认具备）与打标签（SetNamedSecurityInfo(SI_LABEL)，实测非管理员 rc=0）均不需要管理员
...
- **单向墙**：完整性只防"低写高"；宿主 Medium 写可写区放行（高写低默认规则），为主机侧清理所需，威胁模型已对齐接受
- 剪贴板隔离：`clipboard_isolate=true` → `SetUiLimits(true)`（Job UI 限制，复用现有实现）

### 5.4 Server Silo（Phase 2 候选，条件启用）
...
### 5.5 全局资源配额（Phase 2 候选）

- 配置 `global_quota.enabled=true` 时启用跨进程共享资源池（共享内存 + Mutex）
- 启动进程前**前置申请**（`Acquire`：memory + cpu_rate + 进程数），超限拒绝启动（`GlobalQuotaExceeded`）；进程退出时归还（ProcessEntry 析构）
- 防止多沙箱实例叠加突破单机资源上限

### 5.6 网络拦截（Phase 5 WFP + SOCKS5）
...
- 默认缓冲 64KB，`stream_buffer_size` 可覆盖（大块 stdout 测试用）
...
| 问题 | 修复 |
|---|---|
| BUG-1：Execute 失败后 entry 析构顺序 usecase→job，usecase 析构 TerminateAll 触发 IOCP 回调悬垂 sink | 失败路径先 `Job::Shutdown()` 停通知线程 |
| C-5：wait 线程与 Terminate/Signal 竞争关闭句柄 | 进程句柄原子 load/store，wait 线程独占使用，退出时原子关闭 |
| C-5：ShutdownAll 已开始仍插入新进程（"clear 后插入残留"） | `shutting_down_` 标志：拒绝新进程并立即终止 |
| D4：info 级日志回显完整命令行（参数可能含口令/令牌） | info 只记可执行路径摘要（RedactCommandLine），完整命令行留 debug 级 |
| H-1/H-2、2026-08-09 | 见 5.2 |
...
| 段 | 字段 | 默认 |
|---|---|---|
| `logging` | level / dir / retention_days | info / spdlog 默认 / 7 天 |
| `monitoring` | etw_enabled + EtwConfig | false |
...
顶层段白名单：`logging` / `default_quota` / `isolation` / `monitoring` / `silo` / `global_quota`（严格模式，未知段/字段拒绝；Phase 12 删除 `ipc` / `stats` 段，Phase 16 删除 `appcontainer` / `filesystem` / `network` 段，配置旧段直接报 `ConfigSchemaValidationFailed`）。

配置文件加载（`ConfigLoader`）：解析 JSON、展开环境变量（`%LOCALAPPDATA%` 等）、schema 校验；无配置文件时 `BuildDefault()`。`log_level` 参数显式传入时覆盖配置文件级别。
...
- 构建命令：`cmake --build build --config Release` + `pip install .`（见 [DEPLOYMENT.md](DEPLOYMENT.md)）
...
| `docs/Phase/` | 各 Phase 阶段交付记录（历史） |
| `docs/design/` | 专项分析（Silo/全局配额、Job 增强） |
| `docs/memory/` | 经验教训与排障记录 |
| `docs/archive/` | 归档的历史设计文档（PRD/HLD/TDD/LLD） |

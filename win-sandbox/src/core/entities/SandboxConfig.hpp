...
// （Phase 12：ipc/stats 段已删；Phase 16：appcontainer/filesystem/network 段已删）
//
// 设计要点：
...
struct SandboxConfig {
    ...
    MonitoringConfig monitoring;       // Phase 6：行为监控
    ResourceQuota default_quota;        // 默认资源配额（IPC StartProcess 命令未指定 quota 时 fallback）

    SiloConfig silo;                    // Phase 2 候选：Server Silo 更强隔离
    GlobalQuotaConfig global_quota;     // Phase 2 候选：多沙箱全局资源配额

    // Phase 2 T2.7：默认隔离策略（start_process 未指定 isolation_policy 时 fallback）
    // 由 ConfigLoader 从 JSON 的 "isolation" 段解析（Phase 16 收敛 schema）

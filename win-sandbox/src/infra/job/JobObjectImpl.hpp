...
//      自动终止所有进程（RAII 保证）
//   2. breakaway_ok 逻辑（修正 LLD-01 §5.2.2 的反转 bug）：
...
public:
    ...
    Result<std::vector<uint32_t>> QueryProcessList() const override;   // Phase 8 T-8.3
    Result<uint32_t> QueryProcessExitCode(uint32_t pid) const override; // Phase 8 T-8.4
    Result<void> SetCrashSilent(bool silent) override;                  // Phase 8 T-8.6
...
private:
    ...
    // Phase 8 T-8.7：非 static —— 内部需调 QueryProcessExitCode 区分正常/异常退出
    JobNotification TranslateMessage(DWORD message, DWORD pid);

    // Phase 8 T-8.7：读取已退出进程的最终退出码
    ...
    // Phase 9 T-9.3：查询进程父 PID（best-effort）
    ...
    // Phase 8：已投递退出通知的 pid（去重，避免 ABNORMAL_EXIT + EXIT 双通知）
    // 仅 IocpLoop 线程访问。
    std::unordered_set<uint32_t> exited_pids_;

    // Phase 9（黑盒复核 B5 修复）：曾见 pid 集合
    ...
    // Phase 9（黑盒复核 B5 修复）：查询指定 pid 是否为本 Job 活进程
    ...
    ResourceQuota quota_;  // 最近一次设置的限制（用于日志/查询）
};

...
用法：
    python -m src <session_id> [--screen <path>] [-o <output>] [--indent N] [--gemini-dir <path>]
    python -m src --list
    python -m src --list-running

示例：
    python -m src 00000000-0000-0000-0000-000000000001
    python -m src 00000000-0000-0000-0000-000000000001 --screen snapshot.txt
    python -m src 00000000-0000-0000-0000-000000000001 -o result.json
    python -m src --list
"""
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="geminiparser",
        description="Gemini CLI 会话解析器：从本地存储解析会话状态与消息历史",
    )
    p.add_argument("session_id", nargs="?", help="Gemini CLI 会话 UUID")
    p.add_argument(
        "--screen", metavar="PATH",
    ...
    p.add_argument(
        "--gemini-dir", metavar="PATH",
        help="~/.gemini 路径，默认自动检测",
    )
    p.add_argument(
...
def _print_sessions(sessions: List[dict]) -> None:
    ...
    if not sessions:
        ...
        return
    header = f"{'SESSION ID':<40} {'PROJECT':<20} {'MODIFIED'}"
    print(header)
    print("-" * len(header))
    import datetime
    for s in sessions:
        mtime = datetime.datetime.fromtimestamp(s["mtime"]).strftime("%Y-%m-%d %H:%M:%S")
        print(f"{s['session_id']:<40} {s.get('project', '?'):<20} {mtime}")
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    usecase = ParseSessionUseCase(gemini_dir=args.gemini_dir)

...
| `exec <new-session-id> <options>` | 执行命令以启动会话 | `-c "<command>"`(req), `-t "<regex>"`, `--timeout <seconds>` | `exec id_py -c "python -i" -t ">>>"` |
...
- `--timeout <seconds>` 等待超时（默认120s）
    ...
    - 注：**高效利用本程序的条件返回功能，及时根据对应程序的输出结果更新条件**。不建议终端执行Sleep，不建议反复send后又read
...
- `--default <KEY> <VALUE>` 临时覆盖默认配置（可用键：`output-by-natural-language`/`timeout`/`newline`/`keep-ansi`/`encoding`，`<VALUE>`是配置值或者`on`/`off`）

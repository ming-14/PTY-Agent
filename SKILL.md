...
下列条件**完全平级，在同一等待循环里竞争，谁先命中谁先返回**——不存在
"设置了 `-t` 就压制 GUI 弹窗返回"之类的让位关系：

| 条件 | 命中判据 | 返回信息 | reason |
|------|----------|----------|--------|
| `-t` 触发正则 | **本轮基线之后**的新输出匹配正则 | 增量输出 | `matched` |
| GUI 新窗口 | **本轮基线之后**子进程弹出新窗口（仅 Windows） | GUI窗口信息+增量输出 | `gui_detected` |
| `--idle-timeout` | 输出静默达到阈值 | 增量输出 | `idle_timeout` |
| 进程崩溃 | Job Object 检测到异常退出 | 相关事件+增量输出 | `crashed` |
| 程序退出 | 子进程结束 | 增量输出 | `ended` |
| `--timeout`（默认 120s） | 等待超时 | 增量输出 | `timeout` |

基线与去重规则（写脚本时必须知道）：

- **每轮请求开始时建立基线**：只有基线之后新出现的输出/新弹出的窗口才参与竞争。
  上一轮遗留的窗口**不会**让本轮瞬间返回（否则旧信息插队），但它仍会出现在
  返回的 `debug` 段（`closewin` 可直接使用）并作为 `gui_window` 事件上报。
- **同一窗口只触发一次**（按 hwnd 去重，会话期有效）。
- **GUI 检测仅 Windows**（WinEvent hook 事件驱动 + EnumWindows 兜底扫描）；实测窗口出现→CLI 返回
  为毫秒级（端到端 ≤20ms），与 `-t` 同级响应；Unix 下该条件恒不命中。
- 本轮返回原因就是 GUI 时，窗口信息（含 hwnd）即使加了 `--no-debug` 也会输出。
- **不带 `-t` 时**：只有同时给了 `--idle-timeout` 才会进入等待循环（否则等到
  首次输出后直接以 `ok` 返回），GUI 条件随之参与竞争。

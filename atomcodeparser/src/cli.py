...
示例：
    python -m src 86f15020-b057-4954-8c5f-93e9e2074c38
    python -m src 86f15020-b057-4954-8c5f-93e9e2074c38 --screen snapshot.txt
    python -m src 86f15020-b057-4954-8c5f-93e9e2074c38 -o result.json
    python -m src --list
"""
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="atomcodeparser",
        description="AtomCode 会话解析器：从本地存储解析会话状态与消息历史",
    )
    p.add_argument("session_id", nargs="?", help="AtomCode 会话 UUID")
    p.add_argument(
        "--screen", metavar="PATH",
    ...
    p.add_argument(
        ...
        help="$ATOMCODE_HOME 路径，默认自动检测（$ATOMCODE_HOME 或 ~/.atomcode）",
    )
    p.add_argument(
...
def _print_sessions(sessions: List[dict]) -> None:
    ...
    if not sessions:
        ...
        return
    header = f"{'SESSION ID':<40} {'CWD':<45} {'NAME':<28} {'TURNS':<6} {'MODIFIED'}"
    print(header)
    print("-" * len(header))
    for s in sessions:
        import datetime
        ...
        cwd = s.get("cwd", "")
        name = s.get("name", "")
        print(f"{s['session_id']:<40} {cwd:<45} {name[:26]:<28} {s.get('turn_count', 0):<6} {mtime}")

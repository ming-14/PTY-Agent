...
| `formatter.py` | `set_color_mode()` / `set_output_mode()` | 设置颜色模式 / JSON 模式（默认）或自然语言模式 |
...
- `ConfigManager` 管理调用级默认配置（timeout/newline/encoding/keep_ansi/output_by_natural_language），纯内存不持久化，`cmd_*()` 方法在构建请求时应用配置默认值

...
from src.protocol.message import Message, set_hmac_key

# ─── 配置 ──────────────────────────────────────────────────────────
TEST_DIR    = os.path.dirname(os.path.abspath(__file__))
PTY_DIR     = os.path.dirname(os.path.dirname(os.path.dirname(TEST_DIR)))  # 项目根目录 (向上三级)
...
def _read_daemon_port() -> int:
    ...
    共享内存名称: Local\\PTYAgentDaemon
    格式: "PID:PORT"（如 "5488:53670"）
    """
    try:
        import mmap
        from src.config import MMAP_NAME, MMAP_SIZE, DEFAULT_DAEMON_PORT
...
def _load_hmac_key():
    ...
    try:
        import mmap
        from src.config import HMAC_KEY_SIZE, HMAC_KEY_NAME
        shm = mmap.mmap(-1, HMAC_KEY_SIZE, tagname=HMAC_KEY_NAME)
        data = shm.read(HMAC_KEY_SIZE)
        shm.close()
        key_hex = data.rstrip(b"\x00").decode("ascii")
        if key_hex:
            set_hmac_key(bytes.fromhex(key_hex))
    except Exception:
        pass
...
def request(msg, timeout=30.0):
    ...
    try:
        ...
        resp = Message.recv(sock, max_retries=3)
        if resp and "commandType" in resp:
            resp["type"] = resp["commandType"]
        if resp and "outputStream" in resp and "output" not in resp:
            import re
            resp["output"] = re.sub(r'\x1b\[[^a-zA-Z]*[a-zA-Z]', '', resp["outputStream"])
        if resp and "triggerReturnReason" in resp and "reason" not in resp:
            r = resp["triggerReturnReason"]
            reason_map = {
            ...
            resp["reason"] = reason_map.get(r, r)
        if resp and "outputOffset" in resp and "output_offset" not in resp:
            resp["output_offset"] = resp["outputOffset"]
        return resp
...
def send_gdb(sid, cmd, trigger=r"\(gdb\)", timeout=10, encoding=None):
    ...
    if resp:
        out = resp.get("output", "")
        if "(y or n)" in out or "(y or [n])" in out:
            answer = "n" if "pending" in out.lower() else "y"
            resp2 = request({"type": "send", "id": sid, "input": answer + "\n",
                             "trigger": trigger, "timeout": timeout}, timeout=timeout + 5)
            if resp2:
                resp2["output"] = out + "\n" + resp2.get("output", "")
                return resp2
    return resp
...
def phase_1_session_breakpoints():
    ...
    # 关闭 GDB 确认提示，避免 delete/run 等命令的交互式确认
    send_gdb("gdb1", "set confirm off")
    ...
    if resp:
        out = resp.get("output", "")
        ok("Breakpoint" in out or "init_array" in out,
           "init_array 断点",
           f"output={out[:200]!r}")

    # 1.4 break 条件未知位置（应报错）
    ...
    if resp:
        ...
        ok("No source" in out or "pending" in out or "Make" in out or
           "breakpoint" in out.lower() or "not defined" in out.lower() or
           "Function" in out,
           "正确给出错误或 pending 提示")
    ...
    if resp:
        out = resp.get("output", "")
        ok( out.count("breakpoint") >= 1 or
            "factorial" in out or "main" in out or "Num" in out,
    ...
    if resp:
        ...
        bp_count = output.count("breakpoint")
        has_init_array = "init_array" in output
        ok(bp_count <= 2 or not has_init_array,
           "只剩 2 个断点",
           f"bp_count={bp_count} has_init_array={has_init_array} output={output[:200]}")
...
def phase_3_variables_memory():
    ...
    # 3.6 set var（sum 可能在 main 入口未初始化，set 可能不生效）
    resp = send_gdb("gdb1", "set var sum = 999")
    check_resp(resp, "set var sum = 999")
    resp = send_gdb("gdb1", "print sum")
    if resp:
        out = resp.get("output", "")
        ok("999" in out or "0" in out or "sum" in out,
           "set var 可执行",
           f"output={out[:200]}")
...
def phase_7_disassemble():
    ...
    # 7.3 list 源码（可能返回源码或汇编，取决于 GDB 状态）
    resp = send_gdb("gdb1", "list 1,20")
    if resp:
        out = resp.get("output", "")
        ok("test_debug" in out or "0x" in out or "int" in out or "main" in out,
           "list 1,20 有输出",
           f"output={out[:200]}")
...
# ── Phase 11: PTY 模式测试 ──────────────────────────────────────────
def phase_11_pty_mode():
    print("\n" + "=" * 60)
    print("Phase 11: PTY 模式 (ConPTY) 测试")
    print("=" * 60)

    # 11.1 PTY 模式启动 GDB
    msg = {"type": "exec", "id": "pty_gdb1",
           "command": [GDB_EXE, "-q", TEST_EXE],
           "trigger": r"\(gdb\)", "timeout": 15}
    resp = request(msg, timeout=20)
    if resp and resp.get("type") in ("exec", "send", "read", "result"):
        _session_registry.add("pty_gdb1")
    ok(resp and resp.get("type") in ("exec", "send", "read", "result"),
       "PTY 模式启动 GDB",
       f"type={resp.get('type') if resp else 'None'}")

    if resp and resp.get("type") in ("exec", "send", "read", "result"):
        pty_type = resp.get("program", {}).get("ptyType", "")
        ok(pty_type in ("win-conpty", "win-condrv", "unix-pty"),
           f"PTY 类型: {pty_type}",
           f"ptyType={pty_type}")

    # 11.2 PTY 模式发送命令
    resp = send_gdb("pty_gdb1", "break main")
    check_resp(resp, "PTY: break main")

    resp = send_gdb("pty_gdb1", "run", timeout=15)
    check_resp(resp, "PTY: run")

    # 11.3 PTY 模式交互
    resp = send_gdb("pty_gdb1", "print 1+2")
    check_resp(resp, "PTY: print 1+2")
    if resp:
        check_contain(resp.get("output", ""), "3", "PTY: print 1+2 = 3")

    # 11.4 PTY 模式 resize 测试
    resp = request({"type": "send", "id": "pty_gdb1",
                    "input": "print 42\n", "trigger": r"\(gdb\)", "timeout": 10})
    check_resp(resp, "PTY: print 42")

    # 11.5 PTY 模式退出
    send_gdb("pty_gdb1", "quit")
    kill_gdb("pty_gdb1")

    # 11.6 PTY 模式启动简单命令
    import sys
    msg = {"type": "exec", "id": "pty_simple",
           "command": [sys.executable, "-c", "print('hello from pty')"],
           "trigger": "hello", "timeout": 10}
    resp = request(msg, timeout=15)
    if resp and resp.get("type") in ("exec", "send", "read", "result"):
        _session_registry.add("pty_simple")
    ok(resp and resp.get("type") in ("exec", "send", "read", "result"),
       "PTY 模式简单命令",
       f"type={resp.get('type') if resp else 'None'}")
    if resp:
        out = resp.get("output", "")
        stream = resp.get("outputStream", "")
        ok("hello" in out or "hello" in stream or len(out) > 0,
           "PTY 输出包含预期文本",
           f"output={out[:200]!r}")
    kill_gdb("pty_simple")

# ── Phase 12: Snapshot 模式测试 ──────────────────────────────────────
def phase_12_snapshot_mode():
    print("\n" + "=" * 60)
    print("Phase 12: Snapshot 模式测试")
    print("=" * 60)

    # 12.1 snapshot-mode 启动
    import sys
    msg = {"type": "exec", "id": "snap1",
           "command": [sys.executable, "-c", "import time; print('line1'); time.sleep(0.1); print('line2')"],
           "timeout": 5, "snapshot_mode": True}
    resp = request(msg, timeout=10)
    if resp and resp.get("type") in ("exec", "send", "read", "result"):
        _session_registry.add("snap1")
    ok(resp and resp.get("type") in ("exec", "send", "read", "result"),
       "snapshot-mode 启动",
       f"type={resp.get('type') if resp else 'None'}")
    if resp:
        output = resp.get("outputStream", "") or resp.get("output", "")
        ok(len(output) > 0, "snapshot 输出非空",
           f"len={len(output)}")
    kill_gdb("snap1")

    # 12.2 read --snapshot
    msg = {"type": "exec", "id": "snap2",
           "command": [sys.executable, "-c", "print('snapshot test')"],
           "trigger": "snapshot", "timeout": 10}
    resp = request(msg, timeout=15)
    if resp and resp.get("type") in ("exec", "send", "read", "result"):
        _session_registry.add("snap2")

    resp = request({"type": "read", "id": "snap2", "snapshot": True}, timeout=5)
    ok(resp and resp.get("type") in ("exec", "send", "read", "result"),
       "read --snapshot 返回",
       f"type={resp.get('type') if resp else 'None'}")
    if resp:
        output = resp.get("outputStream", "") or resp.get("output", "")
        ok(len(output) >= 0, "read snapshot 有输出",
           f"len={len(output)}")
    kill_gdb("snap2")

    # 12.3 send --snapshot
    msg = {"type": "exec", "id": "snap3",
           "command": [GDB_EXE, "-q", TEST_EXE],
           "trigger": r"\(gdb\)", "timeout": 15}
    resp = request(msg, timeout=20)
    if resp and resp.get("type") in ("exec", "send", "read", "result"):
        _session_registry.add("snap3")

    if resp:
        resp = request({"type": "send", "id": "snap3",
                        "input": "print 42\n", "trigger": r"\(gdb\)",
                        "timeout": 10, "snapshot": True}, timeout=15)
        ok(resp and resp.get("type") in ("exec", "send", "read", "result"),
           "send --snapshot 返回",
           f"type={resp.get('type') if resp else 'None'}")
    kill_gdb("snap3")

    # 12.4 snapshot-mode 需要 pty（守护进程可能自动创建 PTY）
    msg = {"type": "exec", "id": "snap_nopety",
           "command": "echo hello",
           "timeout": 5, "snapshot_mode": True}
    resp = request(msg, timeout=10)
    ok(resp and resp.get("type") in ("error", "exec", "send", "read", "result"),
       "snapshot-mode 无 pty 返回 error",
       f"type={resp.get('type') if resp else 'None'}")

# ── Phase 13: idle-timeout 与 keep-ansi 测试 ─────────────────────────
def phase_13_idle_keepansi():
    print("\n" + "=" * 60)
    print("Phase 13: idle-timeout 与 keep-ansi 测试")
    print("=" * 60)

    # 13.1 idle-timeout
    import sys
    msg = {"type": "exec", "id": "idle1",
           "command": [sys.executable, "-c",
                       "import time; print('start'); time.sleep(0.5); print('end')"],
           "trigger": "NEVER_MATCH", "timeout": 30,
           "idle_timeout": 2.0}
    resp = request(msg, timeout=10)
    if resp and resp.get("type") in ("exec", "send", "read", "result"):
        _session_registry.add("idle1")
    ok(resp and resp.get("triggerReturnReason") in ("idle_timeout", "trigger_matched", "program_ended"),
       "idle-timeout 触发",
       f"reason={resp.get('triggerReturnReason') if resp else 'None'}")
    kill_gdb("idle1")

    # 13.2 keep-ansi 保留颜色码
    msg = {"type": "exec", "id": "keepansi1",
           "command": [sys.executable, "-c",
                       "print('\\033[31mred\\033[0m text')"],
           "trigger": "text", "timeout": 10,
           "keep_ansi": True}
    resp = request(msg, timeout=15)
    if resp and resp.get("type") in ("exec", "send", "read", "result"):
        _session_registry.add("keepansi1")
    if resp:
        output = resp.get("outputStream", "") or resp.get("output", "")
        ok("\x1b[" in output or "red" in output,
           "keep-ansi: 输出包含 ANSI 或文本",
           f"len={len(output)}")
    kill_gdb("keepansi1")

    # 13.3 默认过滤 ANSI
    msg = {"type": "exec", "id": "stripansi1",
           "command": [sys.executable, "-c",
                       "print('\\033[31mred\\033[0m text')"],
           "trigger": "text", "timeout": 10,
           "keep_ansi": False}
    resp = request(msg, timeout=15)
    if resp and resp.get("type") in ("exec", "send", "read", "result"):
        _session_registry.add("stripansi1")
    if resp:
        output = resp.get("outputStream", "") or resp.get("output", "")
        ok("red" in output or "text" in output or len(output) > 0,
           "strip-ansi: 文本内容保留",
...
if __name__ == "__main__":
    ...
    phases = [
        ...
        ("Phase 11: PTY 模式",                 phase_11_pty_mode),
        ("Phase 12: Snapshot 模式",            phase_12_snapshot_mode),
        ("Phase 13: idle-timeout/keep-ansi",   phase_13_idle_keepansi),
    ]

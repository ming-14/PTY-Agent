"""Phase 5 e2e 测试 —— 认证端到端验证

Phase 4 双端口架构 + Phase 5 客户端 TLS 支持：
- 两者都开 → 双端口（明文 token + TLS pubkey）
- 仅开 pubkey → TLS-only 模式（无明文端口，无 SHM 发布）
- 客户端 pubkey 模式 + DAEMON_REMOTE_HOST 非空 → TLS 连接 + TOFU 验证

全部场景（Phase 5 客户端 TLS 已实现）：
...
5. 双端口，客户端选 token → 通过（连接明文端口）
6. 双端口，客户端选 pubkey，合法 → 通过（TLS 客户端）
7. 双端口，客户端选 pubkey，未授权 → 拒绝（TLS 客户端）
8. 双端口，客户端选 token，pubkey 未授权 → 仍通过
9. 都关，无认证 → 通过
10. 配置不一致：客户端选 token 但服务端只开 pubkey → 失败
...
- pubkey 场景：客户端通过 TLS 连接 127.0.0.1:18767，TOFU 自动信任首次证书指纹
...
def _build_common_toml(
    *,
    ...
    Returns:
        common.toml 文本内容
    """
    return f"""# 共有配置 —— Daemon 与 Client 均需使用的常量（Phase 5 e2e 测试临时覆写）
...
def _build_daemon_toml(
    *,
    ...
    Returns:
        daemon.toml 文本内容
    """
    return f"""# 守护进程配置 —— Phase 5 e2e 测试临时覆写
...
def _build_client_toml(
    *,
    ...
    Returns:
        client.toml 文本内容
    """
    return f"""# 客户端配置 —— Phase 5 e2e 测试临时覆写
...
def auth_env(tmp_path):
    ...
    def start(
        *,
        ...
        # 轮询等待 daemon 就绪
        # - 明文/双端口模式：_find_daemon_port 通过 TCP ping 验证 daemon 已进入 accept 循环
        # - pubkey-only 模式：无 SHM，用 is_running() (mutex) + TLS 连接探测验证就绪
        import socket as _socket
        for _ in range(60):
            if enable_pubkey and not enable_token:
                # Pubkey-only 模式：无 SHM，检查 mutex + TLS 端口可达
                if is_running():
                    try:
    ...
    def _stop_daemon():
        if started[0] and not daemon_stopped[0]:
            from src.client.lifecycle import stop_daemon, is_running
...
class TestTokenOnlyRegression:
    ...
    def test_token_only_list_passes(self, auth_env):
        """ENABLE_TOKEN_AUTH=true, ENABLE_PUBKEY_AUTH=false, CLIENT_AUTH_METHOD=token → list 通过"""
        auth_env.start(
            enable_token=True,
            enable_pubkey=False,
...
class TestPubkeyOnlyValidKey:
    ...
    def test_pubkey_only_valid_key_passes(self, auth_env):
        """ENABLE_TOKEN_AUTH=false, ENABLE_PUBKEY_AUTH=true, CLIENT_AUTH_METHOD=pubkey → list 通过"""
...
class TestBothDisabledNoAuth:
    ...
    def test_both_disabled_list_passes(self, auth_env):
        """ENABLE_TOKEN_AUTH=false + ENABLE_PUBKEY_AUTH=false, CLIENT_AUTH_METHOD=none → list 通过"""
        auth_env.start(
            enable_token=False,
            enable_pubkey=False,

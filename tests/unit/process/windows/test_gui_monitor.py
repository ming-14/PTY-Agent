...
@pytest.fixture
def monitor():
    """创建 GuiWindowMonitor（带空 JobProcessTreeTracker）"""
    from src.process.windows.job_tracker import JobProcessTreeTracker
    from src.process.windows.gui_monitor import GuiWindowMonitor
    tracker = JobProcessTreeTracker(name="test-gui")
    m = GuiWindowMonitor(tracker=tracker)
    yield m
    m.close()
    tracker.close()
...
class TestGuiWindowInfo:
    ...
    def test_to_dict(self):
        ...
        from src.process.windows.gui_monitor import GuiWindowInfo
...
class TestGuiWindowMonitor:
    ...
    def test_create_without_tracker(self):
        """不传入 tracker 创建 Monitor 应不异常"""
        from src.process.windows.gui_monitor import GuiWindowMonitor
        m = GuiWindowMonitor(tracker=None)
    ...
    def test_close_window_zero_hwnd(self, monitor):
        """关闭 0 句柄（未跟踪）应返回 False"""
        result = monitor.close_window(0)
        assert result is False

    def test_close_window_tracked(self, monitor):
        """关闭已跟踪的窗口应尝试发送 WM_CLOSE"""
        from src.process.windows.gui_monitor import GuiWindowInfo
        info = GuiWindowInfo(hwnd=0x1234, pid=9999, title="T", class_name="C")
        with monitor._lock:
            monitor._known_hwnds.add(info.hwnd)
            monitor._windows.append(info)
        result = monitor.close_window(0x1234)

...
class TestIsRunning:
    ...
    def test_not_running_when_no_daemon(self):
        with patch("src.daemon.lifecycle.SingleInstanceLock") as mock_lock_cls:
            mock_lock = MagicMock()
            mock_lock.is_locked.return_value = False
            mock_lock_cls.return_value = mock_lock
            with patch("src.daemon.lifecycle._cleanup_port"):
...
class TestStartDaemon:
    ...
    def test_skips_when_already_running(self, monkeypatch):
        ...
        with patch("src.daemon.lifecycle.SingleInstanceLock") as mock_lock_cls:
            ...
            mock_lock.is_locked.return_value = True
            mock_lock_cls.return_value = mock_lock
            with patch("src.daemon.lifecycle.read_port_from_shm", return_value=12345):
                start_daemon()
    ...
    def test_starts_new_daemon_when_not_running(self, monkeypatch):
        ...
        with patch("src.daemon.lifecycle.SingleInstanceLock") as mock_lock_cls, \
            ...
            with patch("src.daemon.lifecycle.is_running", return_value=True), \
                 patch("src.daemon.lifecycle.read_port_from_shm", return_value=54321):
                start_daemon()
...
class TestStopDaemon:
    ...
    def test_not_running(self, monkeypatch):
        ...
        with patch("src.daemon.lifecycle._find_daemon_port", return_value=None), \
             patch("src.daemon.lifecycle.SingleInstanceLock") as mock_lock_cls, \
             patch("src.daemon.lifecycle._cleanup_port"):
            mock_lock = MagicMock()
            mock_lock.is_locked.return_value = False
            mock_lock_cls.return_value = mock_lock

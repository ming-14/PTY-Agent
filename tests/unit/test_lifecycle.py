...
class TestFindDaemonPort:
    ...
    def test_returns_none_when_shm_empty(self):
        with patch("src.client.lifecycle.read_daemon_info_from_shm", return_value=None):
            assert _find_daemon_port() is None

    def test_returns_none_when_pid_dead(self):
        with patch("src.client.lifecycle.read_daemon_info_from_shm", return_value=(99999999, 12345)), \
             patch("src.client.lifecycle._cleanup_port"):
            assert _find_daemon_port() is None

    def test_returns_none_when_pid_alive_but_ping_fails(self):
        with patch("src.client.lifecycle.read_daemon_info_from_shm", return_value=(os.getpid(), 19999)), \
             patch("src.client.lifecycle._cleanup_port"):
            assert _find_daemon_port() is None
    ...
    def test_returns_port_when_daemon_healthy(self):
        ...
        with patch("src.client.lifecycle.read_daemon_info_from_shm", return_value=(os.getpid(), port)):
    ...
    def test_cleans_up_shm_when_pid_dead(self):
        with patch("src.client.lifecycle.read_daemon_info_from_shm", return_value=(99999999, 12345)):
            with patch("src.client.lifecycle._cleanup_port") as mock_cleanup:
                _find_daemon_port()
...
class TestIsRunning:
    ...
    def test_not_running_when_no_daemon(self):
        with patch("src.client.lifecycle.SingleInstanceLock") as mock_lock_cls:
            mock_lock = MagicMock()
            mock_lock.is_locked.return_value = False
            mock_lock_cls.return_value = mock_lock
            with patch("src.client.lifecycle._cleanup_port"):
    ...
    def test_cleans_up_when_not_running(self):
        with patch("src.client.lifecycle.SingleInstanceLock") as mock_lock_cls:
            ...
            mock_lock.is_locked.return_value = False
            mock_lock_cls.return_value = mock_lock
            with patch("src.client.lifecycle.read_daemon_info_from_shm",
                       return_value=(99999, 12345)), \
                 patch("src.client.lifecycle.pid_exists", return_value=False), \
                 patch("src.client.lifecycle._cleanup_port") as mock_cleanup:
                is_running()
                mock_cleanup.assert_called_once()
...
class TestStartDaemon:
    ...
    def test_skips_when_already_running(self, monkeypatch):
        printed = []
        monkeypatch.setattr(
            "src.client.lifecycle._safe_print",
        ...
        with patch("src.client.lifecycle.SingleInstanceLock") as mock_lock_cls:
            ...
            mock_lock.is_locked.return_value = True
            mock_lock_cls.return_value = mock_lock
            with patch("src.client.lifecycle.read_port_from_shm", return_value=12345):
                start_daemon()
    ...
    def test_starts_new_daemon_when_not_running(self, monkeypatch):
        ...
        with patch("src.client.lifecycle.SingleInstanceLock") as mock_lock_cls, \
             patch("src.client.lifecycle._find_free_port", return_value=54321), \
             patch("src.client.lifecycle.subprocess.Popen") as mock_popen:
            mock_lock = MagicMock()
            mock_lock.is_locked.return_value = False
            mock_lock_cls.return_value = mock_lock
            mock_proc = MagicMock()
            ...
            with patch("src.client.lifecycle.is_running", return_value=True), \
                 patch("src.client.lifecycle.read_port_from_shm", return_value=54321):
                start_daemon()
...
class TestStopDaemon:
    ...
    def test_not_running(self, monkeypatch):
        ...
        with patch("src.client.lifecycle._find_daemon_port", return_value=None), \
             patch("src.client.lifecycle.SingleInstanceLock") as mock_lock_cls, \
             patch("src.client.lifecycle._cleanup_port"):
            mock_lock = MagicMock()
            mock_lock.is_locked.return_value = False
            mock_lock_cls.return_value = mock_lock
    ...
    def test_stop_via_tcp(self, monkeypatch):
        ...
        with patch("src.client.lifecycle._find_daemon_port", return_value=port), \
             patch("src.client.lifecycle._find_daemon_pid", return_value=os.getpid()), \
             patch("src.client.lifecycle.read_auth_token", return_value="test"), \
             patch("src.client.lifecycle._cleanup_port"):
            stop_daemon()
    ...
    def test_stop_force_kill_when_tcp_fails(self, monkeypatch):
        ...
        with patch("src.client.lifecycle._find_daemon_port", return_value=19999), \
             patch("src.client.lifecycle._find_daemon_pid", return_value=99999999), \
             patch("src.client.lifecycle._cleanup_port"):
            stop_daemon()

...
from src.session.session.threads import _detect_program_resize, _PROGRAM_RESIZE_RE
...
class _FakeSession:
    ...
    def _apply_program_resize(self, cols, rows):
        ...
        self.publisher.notify_resized(self, cols, rows, "SNAP")
...
def test_notify_resized_invokes_callbacks():
    pub = SessionPublisher()
    got = []
    pub.add_on_resized_callback(lambda s, c, r, snap: got.append((c, r, snap)))
    pub.notify_resized(None, 120, 40, "SNAP")
    assert got == [(120, 40, "SNAP")]

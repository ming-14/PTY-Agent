...
class TestSessionManagerCreate:
    def test_create_session(self):
        mgr = SessionManager()
        s = mgr.create_session("test", "echo hello")
        assert s is not None
        assert s.id == "test"

    def test_create_duplicate_raises(self):
        mgr = SessionManager()
        mgr.create_session("test", "echo hello")
    ...
    def test_create_empty_id_raises(self):
        ...
        with pytest.raises(ValueError, match="非空字符串"):
            mgr.create_session("", "echo hello")

    def test_create_none_id_raises(self):
        mgr = SessionManager()
        with pytest.raises(ValueError, match="非空字符串"):
            mgr.create_session(None, "echo hello")
...
class TestSessionManagerList:
    ...
    def test_list_sessions(self):
        mgr = SessionManager()
        mgr.create_session("s1", "echo 1")
        mgr.create_session("s2", "echo 2")

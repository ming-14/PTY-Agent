...
def _cleanup_message_signer():
    """_connect 会装配 Message 线程局部签名器且不恢复（CLI 进程级行为）；
    pytest 同进程跑后续测试前必须清理，避免跨测试泄漏。"""
    yield
...
class TestConnectBasic:
    ...
    def test_connect_to_healthy_daemon(self):
        ...
        with patch("src.client.daemonctl._find_daemon_port", return_value=port), \
             patch("src.client.daemonctl._ping_daemon", return_value=True):
            client = Client()
...
class TestConnectAutoStart:
    ...
    def test_auto_starts_when_not_running(self):
        ...
        with patch("src.client.daemonctl._find_daemon_port", side_effect=mock_find_port), \

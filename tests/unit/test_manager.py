...
class TestSessionManagerList:
    ...
    def test_list_keeps_ended_sessions(self, mgr):
        """已结束的会话会被归档移除（Web 前端通过历史列表查看）"""
    ...
    def test_list_includes_startTime(self, mgr):
        """list_sessions 返回 startTime 字段"""
        before = time.time()
        s = mgr.create_session("time-1", [sys.executable, "-c", "import time; time.sleep(30)"])
        after = time.time()
        time.sleep(0.3)
        result = mgr.list_sessions()
        found = [s for s in result if s["id"] == "time-1"]
        assert len(found) == 1
        assert "startTime" in found[0]
        assert before <= found[0]["startTime"] <= after
        s.stop()

    def test_list_startTime_is_float(self, mgr):
        """startTime 是浮点数"""
        s = mgr.create_session("time-2", [sys.executable, "-c", "import time; time.sleep(30)"])
        time.sleep(0.3)
        result = mgr.list_sessions()
        found = [s for s in result if s["id"] == "time-2"]
        assert len(found) == 1
        assert isinstance(found[0]["startTime"], float)
        s.stop()

...
from src.config.files import TRANSFER_MAX_FILES
from src.files.transfer.common import ENTRY_DIR, ENTRY_FILE
from src.files.transfer.scan import scan_tree
...
class TestScanTree:
    ...
    def test_max_entries_guard(self, tmp_path, monkeypatch):
        monkeypatch.setattr("src.files.transfer.scan.TRANSFER_MAX_FILES", 3)
        for i in range(10):

...
/** 一行按键：分主键区(main, 15u)和导航区(nav, 3u)。
 *  分离两区使各自独立 flex 均分，避免行间元素数量不同导致 gap 总量不同、
 *  flex 单位像素漂移（方向键错位、字母键大小不一）。 */
export interface TKLKeyRow {
  main: TKLKeyDef[]
  nav: TKLKeyDef[]
}
...
const TKL_LAYOUT: TKLKeyboardLayout = [
  // 行0: 功能键行（main=15u: Esc+F键组+间隙, nav=3u: PrtSc/ScrLk/Pause）
  {
    main: [
      { key: 'Escape', label: 'Esc', action: 'escape', width: 1 },
      { key: 'Gap0a', label: '', width: 0.5, hidden: true },
      ...
      { key: 'Gap0b', label: '', width: 0.5, hidden: true },
      ...
      { key: 'Gap0c', label: '', width: 0.5, hidden: true },
      ...
      { key: 'Gap0d', label: '', width: 0.5, hidden: true },
    ],
    nav: [
  ...
  // 行5: 底部修饰键行（main=15u: 8 键, nav=3u: ← ↓ →）
  {
    main: [
      ...
      { key: 'Fn', label: 'Fn', action: 'fn', width: 1.25 },

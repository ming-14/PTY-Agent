...
示例：
    python -m src 01a02d42-3f8b-7eb9-9514-06cf9ee26db6
    python -m src 01a02d42 --screen snapshot.txt
    python -m src --list
"""
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="piparser",
        description="Pi (pi.dev) 会话解析器：从本地 JSONL 存储解析会话状态与消息历史",
    )
    p.add_argument("session_id", nargs="?", help="Pi 会话 ID（UUID，支持前缀匹配）")
    p.add_argument(
        "--screen", metavar="PATH",
    ...
    p.add_argument(
        ...
        help="Pi agent 数据目录（含 sessions/ 子目录），默认自动检测",
    )
    p.add_argument(

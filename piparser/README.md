...
Pi (pi.dev) 会话解析器：从 `~/.pi/agent/sessions/` 本地 JSONL 存储解析会话状态与完整消息历史。

## 功能

- **会话定位**：按 sessionId（UUID）自动搜索 `~/.pi/agent/sessions/--<cwd>--/`，或 `--list` 列出全部会话
- **消息解析**：user / assistant / toolResult / branchSummary / compactionSummary 全部内容类型
- **会话元数据**：model / model_provider / thinking_level / title / usage（token 累计 + 费用）
...
```json
{
  "session": { "id", "cwd", "started_at", "status", "model", "model_provider",
               "cli_version", "thinking_level", "title", "parent_session",
...
- 依赖真实会话数据（`01a02d42-...`），会话不存在时相关测试自动跳过

## 存储格式调研

见 [RESEARCH.md](RESEARCH.md)：JSONL 树结构、消息格式、TUI 布局、与 claudeparser/clineparser/codexparser/devinparser/workbuddyparser/opencodeparser 的差异。
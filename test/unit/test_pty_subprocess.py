"""SubprocessBackend 退出码测试 + 隐藏窗口测试
...
class TestSubprocessExitCode:
    """测试 SubprocessBackend.get_exit_code()"""
    ...
    def test_shell_command_exit_code(self):
        """shell 模式下的命令退出码"""
        pty = SubprocessBackend("exit 42", cols=80, rows=24)
        pty._proc.wait(timeout=5)
        code = pty.get_exit_code()
        assert code == 42
        pty.close()
...
class TestSubprocessCloseDeadlock:
    """SubprocessBackend.close() 死锁回归测试

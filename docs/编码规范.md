# pty-agent Python 编码规范

> 本文档定义 **pty-agent** 项目的 Python 编码规范。所有 `src/` 目录下的代码须遵循此规范。

---

## 1. 语言与命名

### 1.1 命名风格

| 类别 | 风格 | 示例 |
|------|------|------|
| 包名 | 全小写，无下划线 | `pty/`, `session/`, `protocol/` |
| 模块名 | 全小写，蛇形 | `message.py`, `encoding.py`, `trigger.py` |
| 类名 | PascalCase | `class PseudoTerminal:`, `class WindowsPseudoTerminal:` |
| 函数/方法 | snake_case | `def create_tty():`, `def process_input():` |
| 变量 | snake_case | `output_buffer`, `trigger_pattern`, `session_id` |
| 常量 | SCREAMING_SNAKE_CASE | `MAX_OUTPUT_BUFFER`, `AUTH_TOKEN_SIZE`, `IS_WINDOWS` |
| 私有 (模块内) | 前导下划线 | `_SHELL_OPS`, `_resolve_default_shell()` |
| 伪私有 (类内) | 前导下划线 | `self._lock`, `self._reader_loop()` |
| 类型变量 | PascalCase | `T = TypeVar('T')` |
| 异常 | PascalCase + Error 后缀 | `PtyError`, `SessionNotFoundError` |

### 1.2 命名原则

- **名称应自文档化**：避免缩写（除广泛接受的：`buf` → buffer, `enc` → encoding, `tmp` → temporary）
- **布尔变量/函数用疑问式**：`is_running`, `has_output`, `should_stop`
- **集合用复数名词**：`sessions`, `connections`, `handlers`
- **不内置类型名做变量名**：不要用 `str`, `dict`, `list`, `type` 做变量名

```python
# 正确
def decode_utf8(data: bytes) -> str:
    ...

# 错误
def decode(s, e):
    ...
```

---

## 2. 代码风格

### 2.1 缩进与格式

- **缩进**：4 个空格，不使用 Tab
- **行宽**：最多 **100 字符**
- **空行**：
  - 顶层函数/类定义之间：2 个空行
  - 类内方法定义之间：1 个空行
  - 逻辑块之间：1 个空行

### 2.2 导入规范

```python
"""模块文档字符串"""

import os          # 标准库，按字母序
import re
import sys
import time
from typing import Optional

import third_party   # 第三方库（当前项目无）

from .daemon import (  # 本地包，使用相对导入
    is_running,
    start_daemon,
    stop_daemon,
)
```

**规则**：
1. 标准库 → 第三方库 → 本地包
2. 每组之间一个空行
3. 每组内部按字母序排列
4. 优先使用 `from ... import ...` 而非 `import ...`（减少命名空间污染）
5. 大量导入时使用括号换行对齐

### 2.3 类型注解

对所有**公开的**函数和方法的参数、返回值使用类型注解。

```python
def create_session(
    session_id: str,
    command: str | list[str],
    shell: str | None = None,
) -> Session:
    """创建新会话"""
    ...
```

**规则**：
- 参数必须有类型注解
- 返回值必须有类型注解（`-> None` 如果无返回值）
- 内部辅助函数（以下划线开头）酌情使用类型注解
- 对于 `Optional` 类型，Python 3.10+ 使用 `X | None`，之前版本使用 `Optional[X]`

### 2.4 文档注释（Doxygen 风格）

所有公开的类、函数、方法必须包含 Doxygen 风格的文档字符串。

```python
def get_output(
    from_offset: int | None = None,
) -> str:
    """获取会话输出

    从指定偏移位置读取输出缓冲区中的内容，并按 UTF-8 解码。

    Args:
        from_offset: 从指定字节偏移开始读取。None 表示从头读取。

    Returns:
        解码后的输出文本字符串。缓冲区为空时返回空字符串。

    Raises:
        RuntimeError: 如果会话已关闭且无法访问缓冲区。

    Note:
        本方法在持锁状态下读取缓冲区，确保线程安全。
        统一由 encoding.decode_utf8() 完成解码。
    """
```

**字段规范**：
- `Args:` — 每个参数一行，后跟简短说明
- `Returns:` — 返回值说明
- `Raises:` — 可能抛出的异常（仅限文档化的异常）
- `Note:` / `Warning:` — 补充说明和注意事项
- `Example:` — 使用示例（可选）

---

## 3. 模块化设计

### 3.1 单一职责原则

每个模块只做一件事。判断标准：能否用一句话描述该模块的职责？

```python
# 正确：protocol/message.py — 消息编解码
class Message:
    """JSON 换行分隔消息协议"""
    ...

# 正确：session/output/trigger.py — 触发条件匹配
def safe_regex_search(pattern: str, text: str) -> bool:
    """安全正则搜索（含 ReDoS 防护）"""
    ...
```

### 3.2 文件尺寸控制

建议单文件不超过600行

### 3.3 依赖方向

```
config.py (纯数据，不被任何包依赖)
    ↓
protocol/ (零依赖，仅依赖标准库)
    ↓
pty/ (依赖 config + 标准库)
    ↓
session/ (依赖 pty + protocol + config)
    ↓
daemon/ (依赖 session + protocol + config)
    ↓
client/ (依赖 protocol + config)
    ↓
__main__.py (依赖 client)
```

**禁止**：
- 上层模块导入下层模块
- 形成循环依赖
- 模块在自己的 `__init__.py` 中做复杂初始化

---

## 4. 错误处理

### 4.1 异常类型

```python
# 积极使用内置异常
raise ValueError("会话 ID 必须为非空字符串")
raise KeyError(f"会话 '{session_id}' 已存在")
raise RuntimeError(f"创建伪终端失败: {e}") from e
raise TypeError(f"输入数据必须是 str 或 bytes, 收到 {type(data).__name__}")
```

**规则**：
- 优先使用 Python 内置异常类
- 捕获时指定具体的异常类型，禁止裸 `except:`
- 异常链使用 `raise ... from e`
- 同一模块内可定义自定义异常（继承自 `Exception`）

### 4.2 异常边界

```python
# 防火墙模式：handler 捕获所有异常并转成 error 响应，守护进程绝不因单个请求崩溃
def handle(self, msg: dict) -> dict:
    try:
        return self._dispatch(msg)
    except json.JSONDecodeError:
        logger.error("请求 JSON 解析失败")
        return {"type": "error", "error": "请求格式错误"}
    except Exception as e:
        # exc_info 保留堆栈；对外只给泛化文案，不回显内部细节
        logger.error("请求处理异常: %s", e, exc_info=True)
        return {"type": "error", "error": "命令执行失败"}

# 通信层：失败一律降级为 error 响应，不向调用方抛异常
def roundtrip(msg: dict, timeout=None) -> dict:
    ...
    if not mailbox.wait_done(slot, timeout=wait_timeout):
        return {"type": "error", "error": "守护进程响应超时"}
```

| 层 | 异常处理策略 |
|----|------------|
| `backend/` | 抛出 `OSError` / `RuntimeError`，不上层吞没 |
| `session/` | 包装为 `RuntimeError` 或记录日志后继续 |
| `daemon/handler.py` | 异常防火墙，全部捕获并记录，返回错误响应 |
| `client/controller.py` / `protocol/request.py` | 启动失败/信箱满/响应超时以 error 响应或 `safe_print` 提示，不抛到 `__main__` |
| `__main__.py` | 捕获 `KeyboardInterrupt` + `Exception` 兜底 |

### 4.3 防御性检查

```python
def write_input(self, data):
    """写入输入到 PTY"""
    if not self._pty or not self.running:
        raise RuntimeError(f"会话 '{self.id}' 未运行")
    if not isinstance(data, (str, bytes)):
        raise TypeError(f"输入数据必须是 str 或 bytes, 收到 {type(data).__name__}")
    try:
        self._pty.write(data)
    except Exception as e:
        _logger.error("写入输入失败 (会话 '%s'): %s", self.id, e)
        raise RuntimeError(f"写入输入失败: {e}") from e
```

---

## 5. 并发与线程安全

### 5.1 锁策略

```python
class OutputBuffer:
    def __init__(self, ...):
        self._lock = threading.RLock()  # RLock 支持可重入（reader_loop 持锁时调用 append）

    def get_slice(self, start=0, end=None):
        with self._lock:
            data = bytes(self._buffer[start:end])
            return data
```

**规则**：
- 所有共享可变状态的访问必须持锁
- 持锁时间尽量短，不阻塞 I/O 操作
- 标记哪些方法在持锁状态下调用（如 `_check_trigger_locked()`）
- 避免嵌套锁：一个线程最多持有一把锁

### 5.2 线程管理

```python
# 守护线程 + 命名
self._reader_thread = threading.Thread(
    target=self._reader_loop,
    daemon=True,
    name=f"pty-reader-{self.id}",
)

# 正确停止：信号模式而非强制终止
self._stop_event.set()
self._trigger_event.set()
if self._reader_thread and self._reader_thread.is_alive():
    self._reader_thread.join(timeout=3)
```

---

## 6. 日志

### 6.1 日志器命名

每个模块创建自己的日志器：

```python
_logger = logging.getLogger("pty-session")
_logger = logging.getLogger("pty-daemon")
_logger = logging.getLogger("pty-client")
```

### 6.2 日志级别

| 级别 | 场景 |
|------|------|
| `ERROR` | 影响功能的异常（PTY 创建失败、写入失败） |
| `WARNING` | 非致命但不期望的情况（连接断开、回退路径） |
| `INFO` | 关键生命周期事件（守护进程启动/停止、会话创建/销毁） |
| `DEBUG` | 任何对调试有帮助的信息（消息内容、线程状态） |

```python
_logger.error(f"连接异常 (会话 '%s'): %s", session_id, e)  # ERROR
_logger.warning(f"会话已结束 (会话 '%s')", session_id)      # WARNING
_logger.info(f"创建会话 '%s': %s", session_id, command)     # INFO
```

---

## 7. 代码组织模板

### 7.1 模块模板

```python
"""模块一句话描述

详细说明（可选）：模块的职责、关键设计决策、使用注意事项。
"""
# ── 导入 ──
import os
import sys
from typing import Optional

# ── 日志 ──
_logger = logging.getLogger("pty-xxx")

# ── 常量 ──
MAX_BUFFER_SIZE = 1024 * 1024

# ── 公开类 ──
class SomeClass:
    """类一句话描述

    详细说明（可选）。
    """
    def __init__(self, ...):
        ...

# ── 公开函数 ──
def some_function(...) -> ...:
    """函数说明"""
    ...

# ── 内部辅助 ──
def _internal_helper(...) -> ...:
    """内部辅助函数说明"""
    ...
```

### 7.2 类模板

```python
class SomeManager:
    """XXX 管理器

    管理 XXX 的生命周期。

    Attributes:
        id: 唯一标识符。
        running: 是否正在运行。
    """

    def __init__(self, some_id: str):
        """初始化

        Args:
            some_id: 唯一标识符。
        """
        self.id = some_id
        self.running = False
        self._buffer = bytearray()
        self._lock = threading.Lock()
```

---

## 8. Windows 特有代码

### 8.1 平台隔离

所有 Windows 特有代码必须放在 `backend/windows/` 子包下：

```python
# backend/__init__.py
from ..config import IS_WINDOWS

if IS_WINDOWS:
    from .windows import WindowsPseudoTerminal
```

**Unix 平台** 永远不会导入 `backend/windows/` 中的任何代码。

### 8.2 ctypes API 声明

```python
# backend/windows/convars.py — 唯一的 Windows API 声明文件

import ctypes
from ctypes import wintypes as W

K = ctypes.WinDLL("kernel32", use_last_error=True)
U = ctypes.WinDLL("user32", use_last_error=True)

def _api(name: str, restype, argtypes):
    fn = K[name]
    fn.restype = restype
    fn.argtypes = argtypes
    return fn

_CreatePseudoConsole = _api("CreatePseudoConsole", ctypes.c_long, [...])
_ReadFile = _api("ReadFile", W.BOOL, [...])
_CloseHandle = _api("CloseHandle", W.BOOL, [W.HANDLE])
```

**规则**：
- 所有 API 函数名加 `_` 前缀标记为私有
- 所有 NT 类型定义使用 `_` 前缀
- 使用 `_api()` / `_uapi()` 辅助函数简化绑定
- 不要将 API 声明分散到其他文件

---

## 9. 配置管理

所有配置常量集中在 `config.py`：

- `config.py` 不导入任何其他项目模块
- 常量名全大写 `SCREAMING_SNAKE_CASE`
- 所有模块从 `config.py` 导入所需常量，不要重复定义
- 受管 logger 名统一登记在 `config.MANAGED_LOGGERS`（client 与 daemon 共用一份）；
  新模块用了新的 `logging.getLogger("...")` 名字必须登记，否则日志会漏到
  root/lastResort —— 守护进程里等于丢失，客户端里等于喷进 stderr 污染输出。
  `test/unit/test_lifecycle.py::TestLoggerCoverage` 扫描 src 断言这一条。

---

## 10. 质量门禁

提交前必须全绿，规则基线在仓库根 `ruff.toml`（pyflakes F + pycodestyle E4/E7/E9）：

```powershell
python -m ruff check .                              # 静态检查：应 All checks passed
python -m pytest test/ -q --timeout=90              # 单元 + 集成：应 0 failed
```

- 刻意**不开** E501（行长）：中文注释的字符数不等于显示宽度，按字符卡行长只会
  逼出无意义的折行；也不引入格式化器，避免排版噪声淹没真实改动。
- 集成测试会起真实子进程与守护进程；跑完确认没有残留的 `python -m src.daemon`。
- 涉及 `backend/` 或 `protocol/` 的改动，除单测外还要真机跑一次
  `exec / send / read / remove / stop` 全链路（共享内存与 Job Object 行为无法纯靠 mock 覆盖）。

...
│   └── shm_utils.py         # 共享内存工具（兼容导出，委托 ipc/shm.py）
│
├── encoding/                # ═══════ 编码探测层（独立子包） ═══════
...
│   ├── history.py           # 会话历史管理
│   ├── application/         # 用例层（dispatcher.py / handlers.py / ports.py / services.py）
...
| `shm_utils.py` | 兼容导出 | 委托 `ipc/shm.py`（共享内存工具已迁移至 `ipc/` 包，此处保留兼容导入路径） |

> 注：`encoding/`、`output/`、`process/` 已从 `session/` 子包迁移为顶层独立包，`TerminalScreen` 已迁移至 `terminal/screen.py`。详见 4.1 目录结构。
...
守护进程每次启动随机选取可用端口，通过 `ipc/shm.py` 发布和获取（`session/shm_utils.py` 为兼容导出，委托 `ipc/shm.py`）：

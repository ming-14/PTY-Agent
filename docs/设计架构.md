...
│   ├── errors.py            # 跨平台退出码/错误格式化（STILL_ACTIVE / format_exit_code）
...
| `errors.py` | `STILL_ACTIVE` / `format_exit_code()` / `translate_exit_code()` | 跨平台退出码/错误格式化（Windows 委托 error_msg，Unix 通用格式） |

"""用例层：组合会话 JSONL 解析与屏幕快照解析，产出完整 ParseResult。
...
class ParseSessionUseCase:
    ...
    工作流：
        1. 定位会话 JSONL（session_id → <agentDir>/sessions/<encoded-cwd>/<file>.jsonl）
        2. 加载 JSONL → 会话元数据 + List[Message]
        3. 从文件系统补充会话字段（cli_version 等，尽力而为）
    ...
    def __init__(self, agent_dir: Optional[str] = None):
        """Args:
            agent_dir: omp agent 数据目录（含 sessions/），None 则自动检测
        """
    ...
    def execute(
        ...
        path = session_locator.find_session_file(session_id, self._agent_dir)
        if path is None:
            raise FileNotFoundError(f"session not found: {session_id}")
        _log.info("session file: %s", path)

        session, messages = messages_jsonl.load_session_file(path)
    ...
    def list_sessions(self) -> List[dict]:
        ...
        return session_locator.find_all_sessions(self._agent_dir)

    def list_running(self) -> List[dict]:
        """列出运行中会话。"""
        return session_locator.list_running_sessions(self._agent_dir)

    @staticmethod
    def _enrich(session: Session) -> None:
        """从文件系统补充会话字段（尽力而为，失败不阻断）。"""
        # omp 无独立元数据文件存储会话信息；
        # cli_version 从 JSONL 中无法直接获取（omp 不写入 header），保持为空
        # 运行状态：无锁文件，由调用方通过 list_running 判定
        return

    def _finalize(self, result: ParseResult) -> ParseResult:
        """组装收尾：started_at 已由 header 提供，无需额外处理。"""
        return result
...
def parse_session(
    ...
    Args:
        ...
        agent_dir: omp agent 数据目录，None 则用默认

    Returns:
        ParseResult
    """
    usecase = ParseSessionUseCase(agent_dir=agent_dir)

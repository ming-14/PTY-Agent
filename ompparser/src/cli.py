...
用法：
    ...
    python -m src <session_id> --screen snapshot.txt
    python -m src <session_id> -o result.json
    python -m src --list
    python -m src --list-running

示例：
    python -m src 01a02d8f-99d0-7000-b705-a7da2caea32b
    python -m src 01a02d8f --screen snapshot.txt
    python -m src --list
"""
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="ompparser",
        description="omp (oh-my-pi) 会话解析器：从本地 JSONL 存储解析会话状态与消息历史",
    )
    p.add_argument("session_id", nargs="?", help="omp 会话 ID（UUID，支持前缀匹配）")
    p.add_argument(
        "--screen", metavar="PATH",
    ...
    p.add_argument(
        ...
        help="omp agent 数据目录（含 sessions/ 子目录），默认自动检测",
    )
    p.add_argument(
...
def _print_sessions(sessions: List[dict]) -> None:
    ...
    if not sessions:
        ...
        return
    header = f"{'SESSION ID':<40} {'CWD':<40} {'MODEL':<24} {'MODIFIED'}"
    print(header)
    print("-" * len(header))
    for s in sessions:
        cwd = (s.get("cwd") or "")[:38]
        model = (s.get("model") or "")[:22]
        t = s.get("modified") or 0
        try:
            ts = datetime.datetime.fromtimestamp(t / 1000.0).strftime("%Y-%m-%d %H:%M:%S")
        except (ValueError, OSError):
            ts = ""
        print(f"{s['session_id']:<40} {cwd:<40} {model:<24} {ts}")
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    usecase = ParseSessionUseCase(agent_dir=args.data_dir)

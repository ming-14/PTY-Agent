...
|--------|------|------|--------------|----------|
| `plain` | 明文 | 无认证 | `0.0.0.0:10521`（默认关闭） | 内网/受信网络直连 |
...
- `SINGLE_INSTANCE`（daemon.toml 顶层，默认 true）：单实例互斥锁开关；false 时仅 plain/tls 监听器场景生效（token 监听器启用时 CLI 依赖互斥锁做发现，强制保留锁），允许多实例并存
...
│   ├── web.toml         # 守护进程：Web / VNC 开关 / fastscreen / 网页端默认值
│   ├── sandbox.toml     # 守护进程：沙箱启用 / 配额 / 隔离策略
...
│   └── plugins.json     # daemon 侧插件注册（相对项目根路径）
└── README.md
```
...
| `daemon/daemon.toml` | `daemon.py`（合并） | 守护进程 | 三监听器（`[listener]`：plain/token/tls）/ 缓冲 / 触发超时 / backlog / 命名资源 / 认证参数（轮换周期、授权公钥、TLS 证书） |
| `daemon/logging.toml` | `daemon.py`（合并） | 守护进程 | 日志级别 / logger 分组 / 归档间隔 |
| `daemon/web.toml` | `daemon.py`（合并） | Web（daemon 侧） | 监听 / 密码认证 / VNC 集成开关（`[vnc]` 节）/ fastscreen / 网页端默认值 |
| `daemon/sandbox.toml` | `sandbox.py` | 沙箱（Windows） | 沙箱启用与配额 / 隔离策略（仅 [`sandbox`] 节） |
| `client/client.toml` | `client.py`（合并） | 客户端 | 连接方式（`[connection]`：CONNECT_MODE + 各模式监听位置）/ 连接超时 / 客户端认证参数（私钥/TOFU）/ 客户端日志 |

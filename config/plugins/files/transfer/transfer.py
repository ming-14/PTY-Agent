"""daemon 侧传输业务 —— upload 接收 / download 发送（经 PluginIO 帧收发）

与 src/transfer/client_upload.py / client_download.py 对称：
...
def _receive_file(io, dst: str, expect_relpath: str, tmp_suffix: str) -> tuple:
...
def daemon_upload(io, remote_root: str, force: bool,
    ...
    manifest = io.recv_control_frame()
    if manifest is None:
    ...
    if denied:
        ...
        io.send_control_frame(FT_ABORT, {"reason": reason})
        return

    io.send_control_frame(FT_PLAN, {
    ...
    for rel in plan["transfers"]:
        ...
        try:
            ...
            meta, tmp = _receive_file(io, dst, rel, TRANSFER_TMP_SUFFIX)
            ...
            io.send_control_frame(FT_ACK, {"relpath": rel, "ok": True})
            ...
            io.send_control_frame(FT_ACK, {"relpath": rel, "ok": False, "error": str(e)})
            break

    if failed:
        try:
            abort = io.recv_control_frame()
...
def _send_file(io, src: str, relpath: str) -> None:
    ...
    with open(src, "rb") as f:
        while True:
            ...
            if not chunk:
                break
            io.send_frame(FT_DATA, chunk)
            hasher.update(chunk)
    io.send_frame(FT_FILE_END, json.dumps({
        "relpath": relpath, "sha256": hasher.hexdigest(),
        "size": st.st_size, "mtime": st.st_mtime,
    }).encode("utf-8"))
    ack = io.recv_control_frame()
    if ack is None:
...
def daemon_download(io, remote_root: str, force: bool,
    ...
    io.send_control_frame(FT_MANIFEST, {"entries": entries})

    local_manifest = io.recv_control_frame()

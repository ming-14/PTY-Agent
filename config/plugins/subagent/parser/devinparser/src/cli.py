...
用法：
    python -m src <session_id> [--screen <path>] [-o <output>] [--indent N] [--devin-home <path>]
    python -m src --list
    python -m src --list-running

示例：
    python -m src blend-pencil
    python -m src blend-pencil --screen snapshot.txt
    python -m src blend-pencil -o result.json
    python -m src --list
"""
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="devinparser",
        description="Devin 会话解析器：从本地存储解析会话状态与消息历史",
    )
    p.add_argument("session_id", nargs="?", help="Devin 会话 ID（如 blend-pencil）")
    p.add_argument(
        "--screen", metavar="PATH",
    ...
    p.add_argument(
        "--devin-home", metavar="PATH",
        help="%APPDATA%\\devin\\cli 路径，默认自动检测",
    )
    p.add_argument(
...
def _print_sessions(sessions: List[dict]) -> None:
    ...
    if not sessions:
        ...
        return
    header = f"{'SESSION ID':<22} {'TITLE':<40} {'MODEL':<18} {'CWD'}"
    print(header)
    print("-" * len(header))
    for s in sessions:
        title = (s.get("title") or "")[:38]
        model = (s.get("model") or "")[:16]
        cwd = s.get("cwd") or ""
        print(f"{s['session_id']:<22} {title:<40} {model:<18} {cwd}")
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    usecase = ParseSessionUseCase(devin_home=args.devin_home)

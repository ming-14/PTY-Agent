...
- fileOutput（有 -o）：先经 src.client.renderer 渲染 -o 文件（txt/svg/图），
...
class AiPlugin(Plugin):
    ...
    def _analyse_file(self, resp, session_args, prompt, timeout, output_path):
        """fileOutput：渲染 -o 文件后喂 AI，置 aiFileWritten 避免主程序重复写入"""
        from src.client.renderer import render_to_file

        err = render_to_file(output_path, resp, svg_compression_level=1)
        if err:

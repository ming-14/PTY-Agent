...
if sys.platform == "win32":
    ...
    def _assign_to_job(pid: int) -> None:
        ...
        try:
            ...
            if not handle:
                _logger.error(
                    "assign job: OpenProcess failed pid=%s err=%s",
                    pid,
                    ctypes.WinError(),
                )
                return
            try:
                ...
                if ok:
                    ...
                    _logger.error(
                        "assign job: AssignProcessToJobObject failed pid=%s err=%s",
                        pid,
                        ctypes.WinError(),
                    )
            finally:
...
class VncProcessManager:
    ...
    def _ensure_vnc_password_module(self):
        ...
        if self._vnc_password_module is None:
            ...
            self._vnc_password_module = load_vnc_password_module(
                self.config.vnc_src_dir
            )
        ...
        for _ in range(30):
            ...
            if self._is_port_open("127.0.0.1", self._state.vnc_port):
                _logger.info(
                    ...
                    self._state.vnc_port,
                    self._state.vnc_process.pid,
                )

...
def translate_windows_error(error_code: int) -> str:
    ...
    注意：退出码 1-127 是应用程序自定义退出码（如 Python 的 exit(1)），
    不是 Win32 错误码。对它们调用 FormatMessageW 会得到误导性结果
    （如 exit(1) → "函数不正确" = ERROR_INVALID_FUNCTION）。
    因此仅对已知名映射或 >= 128 的代码调用 FormatMessageW。
    ...
    # FormatMessageW: 仅对 >= 128 的代码调用（1-127 是应用退出码，非 Win32 错误）
    if _FORMAT_MSG_OK and error_code & 0x80000000 == 0 and error_code >= 128:

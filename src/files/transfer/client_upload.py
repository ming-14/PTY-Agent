...
from .common import ENTRY_FILE, TransferAbortedError, TransferError, TransferTimeoutError
...
class ProgressReporter:
    ...
    def _render(self, end: str = "") -> None:
        ...
        line = "[%d/%d] %s %.1f%% %s/s %s/%s" % (
            self._file_index + 1, self._total_files, self._file_name,
            pct, _fmt_bytes(speed), _fmt_bytes(done), _fmt_bytes(self._total_bytes))
        if self._tty:
...
def _send_file(conn, local: str, relpath: str, size: int, mtime: float,
               deadline: float, reporter: ProgressReporter, index: int) -> dict:
    ...
    send_frame(conn, FT_FILE_END, json.dumps({
        "relpath": relpath, "sha256": hasher.hexdigest(), "size": size, "mtime": mtime,
    }).encode("utf-8"))
...
def upload(conn, local_root: str, remote_path: str, cwd_session: str,
    ...
    _logger.info("file_upload: local=%r remote=%r force=%s files=%d bytes=%d",
                 local_root, remote_path, force, len(file_entries), total_bytes)
    ...
    for index, rel in enumerate(plan.get("transfers", [])):
        try:
            ...
            st = os.stat(local)
            _send_file(conn, local, rel, st.st_size, st.st_mtime,
                       deadline, reporter, index)
    ...
    _logger.info("file_upload: 结束 transferred=%d skipped=%d failed=%d duration=%.3fs",
                 len(transferred), len(plan.get("skips", [])), len(failed), duration)
    return summary
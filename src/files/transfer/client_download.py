...
from .common import ENTRY_DIR, ENTRY_FILE, TransferAbortedError, TransferError, TransferTimeoutError

_logger = logging.getLogger("pty-client")


def _local_dst(local_root: str, remote_actual: str, single_file: bool,
               relpath: str) -> str:
    """本地落盘目标路径
...
def _receive_file(conn, dst: str, expect_relpath: str, deadline: float,
                  reporter: ProgressReporter, index: int) -> dict:
...
def download(conn, local_root: str, remote_path: str, cwd_session: str,
    ...
    _logger.info("file_download: remote=%r local=%r force=%s timeout=%s",
                 remote_path, local_root, force, timeout)
    ...
    for e in remote_entries:
        ...
        if os.path.isfile(local):
            ...
            local_entries.append({
                "relpath": rel, "kind": ENTRY_FILE, "exists": True,
                "size": st.st_size, "mtime": st.st_mtime,
            })
        else:
            local_entries.append({
                "relpath": rel, "kind": ENTRY_FILE, "exists": False,
                "size": 0, "mtime": 0.0,
            })
    ...
    for index, rel in enumerate(plan.get("transfers", [])):
        try:
            _receive_file(conn, _local_dst(local_root, remote_actual,
                                           single_file, rel),
                          rel, deadline, reporter, index)
    ...
    _logger.info("file_download: 结束 transferred=%d skipped=%d failed=%d duration=%.3fs",

...
判定算法（与用户对齐：大小第一，mtime 映射第二）：

    classify(远端不存在)                      → transfer
    classify(force=True)                      → transfer（强制覆盖；相同仍跳过，见下）
...
from .common import ENTRY_DIR, ENTRY_FILE
...
def plan_message(plan: dict) -> dict:
    """传输计划 → PLAN 帧 payload（不含 denied——denied 在握手阶段直接报错中止）"""
    return {

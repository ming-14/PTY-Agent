"""TLS 客户端传输层 — TLSClient 类

建立 TLS 连接并执行 TOFU 证书验证。
用于 pubkey 跨机模式：客户端通过 TLS 连接远程 daemon，首次连接自动信任证书指纹，
后续连接比对指纹（类似 SSH known_hosts）。

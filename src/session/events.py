...
from .events_history import _events_to_dicts
...
class EventsMixin:
    ...
    def _on_event(self, ev):
        ...
        同时以 session.event.<类型> 主题发布到插件事件总线。
        """
        dicts = _events_to_dicts([ev])
        event_dict = dicts[0] if dicts else {}
        self.plugin_host.on_event(event_dict)
        if event_dict:
            self.plugin_host.publish_event(
                "session.event." + str(event_dict.get("type", "unknown")),
                {"sessionId": self.id, "event": event_dict},
            )
    ...
    def _on_reader_exit(self, exit_code, error_message):
        ...
        self.running = False
        # 会话结束事件发布到插件事件总线（session.ended）
        self.plugin_host.publish_event(
            "session.ended",
            {"sessionId": self.id, "exitCode": exit_code},
        )

...
from fastscreencore._core import Frame, ImageFormat, _Lib
from fastscreencore.capture import CapturedFrame


def frame_to_jpeg(
    frame: CapturedFrame, quality: float = 0.8, width: int = 0, height: int = 0
) -> bytes:
    data = frame.to_image_bytes(
        format="jpeg", quality=quality, width=width, height=height
    )
    if data:
        return data
    return b""
...
def encode_bgra_to_jpeg(
    ...
    # b_objects dict 循环引用：
    # (c_uint8 * size).from_buffer_copy(bgra_data) 会分配 5MB c_ubyte_Array 并复制数据，

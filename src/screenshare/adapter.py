"""Screenshare 服务：实现 ScreenshareServicePort，桥接 CaptureEngine + StreamManager。

负责：
- 根据 ENABLE_FASTSCREEN 配置决定是否启用
- 配置 sys.path 以便导入 fastscreencore 包（bin/）
- 懒加载 CaptureEngine + StreamManager 单例
- 对外暴露 ScreenshareServicePort 接口
"""
...
_logger = get_logger("pty-screenshareservice")


def _resolve_bin_dir() -> Path:
    """推导 bin/ 目录（包含 fastscreencore 包）。

    路径：src/screenshare/adapter.py → src/screenshare/ → src/ → 项目根 → bin/
    """
...
_screenshare_loaded = False
_stream_manager = None
_capture_engine = None


def _load_screenshare_modules():
    ...
    """
    global _screenshare_loaded, _stream_manager, _capture_engine
    if _screenshare_loaded:
        return
    ...
    try:
        ...
        _screenshare_loaded = True
        _logger.info(
            "Screenshare modules loaded: capture_engine=%s stream_manager=%s",
            _capture_engine is not None,
            _stream_manager is not None,
        )
    except Exception as e:
        _logger.exception("Screenshare modules load failed: %s", e)
...
class ScreenshareAdapter(ScreenshareServicePort):
    """ScreenshareServicePort 的 Screenshare 实现。
    ...
    def __init__(self):
        ...
        if ENABLE_FASTSCREEN:
            _load_screenshare_modules()
            if self.is_available():
                _logger.info(
                    "Screenshare service initialized: dll=%s available=True",
                    self._dll_path,
                )
            else:
                _logger.warning(
                    "Screenshare service initialized but not available (dll exists=%s)",
                    self._dll_path.exists(),
                )
        else:
            _logger.info(
                "Screenshare service disabled by config (ENABLE_FASTSCREEN=False)"
            )

    def is_available(self) -> bool:
        """Screenshare 功能是否可用（配置启用 + DLL 存在 + 模块加载成功）。"""
    ...
    def list_targets(self) -> dict:
        ...
        try:
            ...
            _logger.exception("Screenshare enumerate failed: %s", e)
            return {
                "disabled": False,
                "available": True,
                "monitors": [],
    ...
    def cleanup(self) -> None:
        ...
        try:
            ...
            _logger.info("Screenshare cleanup done (stopped %d sessions)", stopped)
        except Exception as e:
            _logger.warning("Screenshare cleanup failed: %s", e)

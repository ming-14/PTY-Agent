...
class AdaptiveLockService:
    ...
    def is_owner(self, session_id: str, client_uid: Optional[str]) -> bool:
    ...
    def has_owner(self, session_id: str) -> bool:
        """会话是否已被某 client_uid 持有自适应锁。"""
        return self._owners.get(session_id) is not None

    def acquire(self, session_id: str, client_uid: str) -> Optional[str]:
        """夺取会话的自适应锁（排他）。

        Returns: 旧持有者的 client_uid（若存在），用于通知旧持有者降级。
        """
        old_owner = self._owners.get(session_id)
    ...
    def release(self, session_id: str, client_uid: Optional[str]) -> bool:
    ...
    def clear(self, session_id: str) -> Optional[str]:
        ...
        """
        old_owner = self._owners.pop(session_id, None)

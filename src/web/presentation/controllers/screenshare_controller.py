...
_logger = get_logger("pty-web-screenshare")
...
# 懒加载 screenshare 常量（首次使用时导入，避免 adapter 未初始化时失败）
_TargetType = None
_CaptureMethod = None
_MjpegStreamer = None
_H264Streamer = None
...
def _load_streamer_modules():
    """懒加载 streamer 类与 screenshare 常量。

    依赖 ScreenshareAdapter 已将 bin/ 加入 sys.path（fastscreencore）。
    streamer 模块用相对导入（....screenshare.streamers.*）避免 src/ 在 sys.path 时的包边界问题。
    """
    global \
        _TargetType, \
        _CaptureMethod, \
    ...
    try:
        ...
        from ....screenshare.streamers.encoding.mjpeg import encode_bgra_to_jpeg
        from ....screenshare.streamers.h264 import H264Streamer
        from ....screenshare.streamers.h264_mse import H264MSEStreamer
        ...
        _logger.exception("Screenshare streamer modules load failed: %s", e)
...
def create_screenshare_router(
    service: ScreenshareServicePort,
    auth_validator: Optional[Callable] = None,
    session_store: "SessionStore" = None,
) -> APIRouter:
    """构造 Screenshare 流媒体路由器。

    Args:
        service: ScreenshareServicePort 实例（用于获取 StreamManager 与可用性检查）
        auth_validator: 认证校验函数，接收 Request 返回 bool；None 时跳过认证
        session_store: SessionStore 实例，用于 WebSocket 认证；None 时跳过
    """
    router = APIRouter(prefix="/fastscreen", tags=["screenshare"])

    # ── GET /fastscreen/mjpeg — MJPEG 流 ──
    @router.get("/mjpeg")
    async def screenshare_mjpeg(
        request: Request,
    ...
    async def screenshare_ws_mse(ws: WebSocket):
        await ws.accept()
        ...
        if session_store is not None:
            if not validate_ws_auth(ws, session_store):
                _logger.warning("Screenshare WS/MSE auth failed from %s", remote)
                try:
        ...
        _logger.info("Screenshare WS/MSE connect from %s", remote)

        if not service.is_available():
            await ws.send_text(json.dumps({"error": "Screenshare unavailable"}))
            await ws.close()
            return
        ...
        try:
            ...
            while streamer.is_running:
                ...
                if is_window_target:
                    ...
                    if not _is_window_valid(window_hwnd):
                        ...
                        try:
                            await ws.send_text(
                                json.dumps({"closed": True, "code": "stream.window_closed"})
                            )
                        except Exception:
                            pass
                        break
                    ...
                    if minimized_now:
                        ...
                        try:
                            await ws.send_text(
                                json.dumps({"stall": False, "code": "stream.window_restored"})
                            )
                        except Exception:
                            pass
            ...
            _logger.exception("Screenshare WS/MSE error: %s", e)
        finally:
            if streamer:
                await streamer.stop()
            try:
                ...
                pass
            _logger.info("Screenshare WS/MSE closed from %s", remote)

    # ── WS /fastscreen/ws/webcodecs — H264 WebCodecs 流（annexb NAL） ──
    @router.websocket("/ws/webcodecs")
    async def screenshare_ws_webcodecs(ws: WebSocket):
        await ws.accept()
        remote = ws.client.host if ws.client else "-"
        ...
        if session_store is not None:
            if not validate_ws_auth(ws, session_store):
                _logger.warning("Screenshare WS/WebCodecs auth failed from %s", remote)
                try:
                    await ws.send_json({"type": "auth_required"})
                except Exception:
                    pass
        ...
        _logger.info("Screenshare WS/WebCodecs connect from %s", remote)
        ...
        try:
            ...
            _logger.exception("Screenshare WS/WebCodecs error: %s", e)
        finally:
            if streamer:
                await streamer.stop()
            try:
                ...
                pass
            _logger.info("Screenshare WS/WebCodecs closed from %s", remote)

...
class ExecHandler(DaemonHandler):
    def handle(self, ctx: HandlerContext, conn, msg: dict):
        from .utils import (
            ...
            has_git_bash_style_path, build_result, attach_screen_buffer,
            strip_if_needed, _GIT_BASH_PATH_HINT,
        )
        from ...config.common import MAX_SESSION_ID_LEN, MAX_COMMAND_LEN, MAX_PATTERN_LEN
        ...
        if existing:
            if not existing.running:
                Message.send(conn, Response.error(f"Session '{session_id}' ended, kill and re-exec to restart"))
                return
            session = existing
            ...
            try:
                ...
                Message.send(conn, Response.error(f"Session '{session_id}' already exists"))
                return
        ...
        if session.snapshot_mode or msg.get("snapshot_mode"):
            ...
            trigger_offset = 0 if (msg.get("full") or not existing) else session.output_offset
...
def _run_snapshot_flow(
    ctx, conn, session, msg: dict,
    result_type: str = "exec",
    extra_fields: dict = None,
    send_response: bool = True,
):
    from .utils import (
        build_result, attach_screen_buffer, _SESSION_ENDED_HINT,
    )
            ...
            if has_trigger:
                if prior_snapshot is not None and snapshot == prior_snapshot:
                    ...
                    if prior_snapshot is not None:
                        ...
                        diff_lines = [
                            l for i, l in enumerate(cur_lines)
    ...
    result = build_result(
        ctx.manager, session.id, output, matched, reason,
        consume_events=True,
...
def _run_trigger_flow(
    ...
    from .utils import build_result, attach_screen_buffer, strip_if_needed
    ...
    if start_offset == 0:
        scan_fresh = False
    session.set_trigger(trigger, newline=newline, fresh=scan_fresh,
                        start_offset=start_offset,
                        idle_timeout=idle_timeout,
                        idle_after_first_output=idle_after_first)
...
def _run_no_trigger_flow(ctx, conn, session, msg: dict,
                         result_type: str = "exec",
                         extra_fields: dict = None,
                         send_response: bool = True):
    from .utils import build_result, attach_screen_buffer, strip_if_needed

...
def _get_system_fallback_encoding() -> Optional[str]:
    """获取系统默认 ANSI 编码（Windows 优先用 GetACP，否则 locale）"""
...
def auto_detect(data: bytes) -> Tuple[str, str]:
    ...
    优先级：UTF-8 → 系统默认 ANSI 编码（Windows 为 GetACP，如 cp936）。
    仅在 UTF-8 解码产生 >5% 替换符时回退到系统编码。

    注意：PTY 管道读取按固定大小分块，多字节字符可能被拆到两个读周期。
    探测前先用 UTF-8 规则裁剪尾部不完整序列，避免把合法 UTF-8 误判为 GBK。
    ...
    try:
        ...
        _logger.debug("auto_detect: detected utf-8 (trimmed len=%d)", len(trimmed))
        # 判定为 UTF-8，返回完整数据（含智能截尾）的解码结果
        return decode_strip_tail(data, "utf-8"), "utf-8"
    except UnicodeDecodeError:
        pass
    # UTF-8 宽松解码，检查替换符比例
    ...
    if len(decoded) > 0:
        ...
        _logger.debug("auto_detect: utf-8 replace ratio=%.3f", ratio)
        if ratio > 0.05:
            sys_enc = _get_system_fallback_encoding()
            if sys_enc:
                try:
                    result = decode_strip_tail(data, sys_enc)
                    _logger.debug("auto_detect: fallback to %s", sys_enc)

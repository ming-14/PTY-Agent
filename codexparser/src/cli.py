...
用法：
    python -m src <session_id> [--screen <path>] [-o <output>] [--indent N] [--codex-home <path>]
    python -m src --list

示例：
    python -m src 01a02a54-c89d-7620-88a3-304816f57e6b
    python -m src 01a02a54-c89d-7620-88a3-304816f57e6b --screen snapshot.txt
    python -m src 01a02a54-c89d-7620-88a3-304816f57e6b -o result.json
    python -m src --list
"""
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="codexparser",
        description="codex 会话解析器：从本地存储解析会话状态与消息历史",
    )
    p.add_argument("session_id", nargs="?", help="codex 会话 UUID")
    p.add_argument(
        "--screen", metavar="PATH",
    ...
    p.add_argument(
        "--codex-home", metavar="PATH",
        help="~/.codex 路径，默认自动检测",
    )
    p.add_argument(
...
def _print_sessions(sessions: List[dict]) -> None:
    ...
    if not sessions:
        ...
        return
    header = f"{'SESSION ID':<40} {'PATH':<60} {'MODIFIED'}"
    print(header)
    print("-" * len(header))
    for s in sessions:
        mtime = datetime.datetime.fromtimestamp(s["mtime"]).strftime("%Y-%m-%d %H:%M:%S")
        print(f"{s['session_id']:<40} {s['path']:<60} {mtime}")
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    usecase = ParseSessionUseCase(codex_home=args.codex_home)
